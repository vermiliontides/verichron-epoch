...
function isStageManifest(value) {
    if (typeof value !== "object" || value === null)
        return false;
    const v = value;
    return (typeof v.entrypoint === "string" &&
        (v.runtime === "python" || v.runtime === "node") &&
        typeof v.order === "number" &&
        typeof v.requiresResultsPath === "boolean" &&
        typeof v.enabled === "boolean");
}
async function loadStageFromDir(dir, name) {
    ...
    try {
        ...
        throw new Error(`[orchestrator] ${manifestPath} is not valid JSON: ${err.message}`);
    }
    if (!isStageManifest(parsed)) {
        throw new Error(`[orchestrator] ${manifestPath} does not match stage-manifest.schema.json ` +
...
async function discoverStages() {
    ...
    const stages = loaded.filter((s) => s !== null && s.manifest.enabled);
    ...
    const maxExtractorOrder = Math.max(0, ...stages.filter((s) => s.dir !== REPORTING_DIR).map((s) => s.manifest.order));
    if (reportStage && reportStage.manifest.order <= maxExtractorOrder) {
        throw new Error(`[orchestrator] apps/reporting/stage.json declares order ${reportStage.manifest.order}, ` +
...
function deriveResultsPath(backupPath) {
...
async function hasSucceededRun(client, backupPath) {
    const { rows } = await client.query(`SELECT pr.run_id
...
async function createRun(client, backupPath, stages) {
    const runId = randomUUID();
    await client.query(`INSERT INTO pipeline_runs (run_id, backup_source) VALUES ($1, $2)`, [runId, backupPath]);
    for (const stage of stages) {
        await client.query(`INSERT INTO pipeline_stage_status (run_id, stage_name, status) VALUES ($1, $2, 'pending')`, [runId, stage.name]);
    }
    return runId;
}
async function markStage(client, runId, stageName, status, errorMessage) {
    const timestampCol = status === "running" ? "started_at" : "finished_at";
    await client.query(`UPDATE pipeline_stage_status
     SET status = $1, error_message = $2, ${timestampCol} = now()
     WHERE run_id = $3 AND stage_name = $4`, [status, errorMessage ?? null, runId, stageName]);
}
function runStage(stage, config, runId) {
...
async function runPipelineForBackup(client, backupPath, dbUrl, pythonBin, stages) {
