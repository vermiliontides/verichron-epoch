...
function parseFlags() {
    ...
    try {
        iocMaxAgeMs = parseDuration(values["ioc-max-age"]);
    }
    catch (err) {
    ...
    return {
        ...
        mvtBin: values["mvt-bin"],
        sqliteBin: values["sqlite-bin"],
        force: values.force,
        forceDecrypt: values["force-decrypt"],
        refreshIOCs: values["refresh-iocs"],
        iocMaxAgeMs,
        only: values.only,
        samePass: !values["different-passwords"],
    };
}
function printUsage() {
...
async function run(cfg) {
    ...
    if (backups.length === 0) {
        throw new Error(`no backup directories found under ${cfg.source} (looked for Manifest.db / Info.plist up to ${BACKUP_SEARCH_MAX_DEPTH} levels deep)`);
    }
    ...
    const repairFailuresByBackup = new Map();
        ...
        if (!decryptRan && (await pathExists(repairMarker))) {
            ...
            try {
                ...
                if (result.scanned === null) {
                    ...
                    console.log(`  [repair]  done, repaired ${result.repaired} DB(s)${result.failed > 0 ? `, ${result.failed} could not be fully recovered` : ""} (scanned ${result.scanned} candidate file(s))`);
                }
                if (result.scanned !== null) {
...
async function discoverBackups(source, only) {
    let wanted = null;
    if (only !== "") {
    ...
    for (const e of topEntries) {
        if (!e.isDirectory())
            continue;
        if (wanted !== null && !wanted.has(e.name))
            continue;
...
async function findBackupRoots(dir, remainingDepth) {
...
async function isBackupRoot(dir) {
...
async function readRepairFailures(p) {
    try {
...
// integrity.
async function hashBackup(cfg, name, src) {
...
async function decryptBackup(cfg, src, dest, password) {
    await fsp.mkdir(dest, { recursive: true });
    await runInherited(cfg.mvtBin, ["decrypt-backup", "-p", password, "-d", dest, src]);
}
async function checkBackup(cfg, decryptedDir, resultsDir, logPath) {
    ...
    await new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            if (code === 0)
                ...
                reject(new Error(`mvt-ios check-backup exited with code ${code}`));
        });
    });
}
...
async function repairDecrypted(cfg, decDir) {
    if (!sqliteBinChecked) {
    ...
    for await (const p of walkFiles(decDir)) {
        if (path.basename(p).startsWith(".mvt_"))
            continue; // our own markers
        if (p.endsWith("-wal") || p.endsWith("-shm") || p.includes(".corrupt-"))
            continue;
        if (!(await looksLikeSqlite(p)))
            continue;
        scanned++;
        ...
        try {
            ...
            if (okNow) {
                console.log(`  [repair]  recovered: ${path.relative(decDir, p)}` +
                    (applyWarnings
                ...
                console.warn(`  [repair]  WARNING: ran .recover on ${path.relative(decDir, p)} but it still fails quick_check ` +
...
async function checkSqliteBinAvailable(sqliteBin) {
...
async function looksLikeSqlite(p) {
    let fh;
    try {
...
function sqliteQuickCheck(sqliteBin, dbPath) {
    return new Promise((resolve) => {
...
function summarizeSqliteWarnings(stderr) {
    const lines = stderr
        .split("\n")
        .map((l) => l.trim())
        .filter(Boolean);
...
async function sqliteRecoverInPlace(sqliteBin, dbPath) {
    const recoverResult = await runProcess(sqliteBin, [dbPath, ".recover"]);
    if (recoverResult.stdout.trim() === "") {
        throw new Error(`sqlite3 .recover produced no output` + (recoverResult.stderr.trim() ? `: ${recoverResult.stderr.trim()}` : ""));
    }
    ...
    if (!(await pathExists(repairedPath))) {
        await fsp.rm(repairedPath, { force: true });
        throw new Error(`sqlite3 did not produce a repaired file` + (applyResult.stderr.trim() ? `: ${applyResult.stderr.trim()}` : ""));
    }
...
async function writeSummary(cfg, backups, repairFailuresByBackup) {
    ...
    for (const backup of backups) {
        ...
        if (repairFailures.length > 0) {
            parts.push(`⚠️ ${repairFailures.length} database${repairFailures.length > 1 ? "s" : ""} could not be fully recovered ` +
        ...
        if (hits.length === 0) {
            parts.push("No indicator/warning lines found in log. **Absence of a match is not proof of a clean device** " +
